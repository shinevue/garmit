 ...
 * 
 * LineConnectionSearchForm Reactコンポーネント
 * 
 */
'use strict';
...
import { getSelectedInPatchCable, getInPatchCables, getLineConnections, getSelectedRelPatchCables, getSelectedEndIdfPatchCables } from 'projectLineUtility';
import { getChangeInPatchborad, getChangeInPatchNo } from 'projectLineUtility';
import { VALIDATE_STATE } from 'inputCheck';

/**
 * 使用中の線番を検索する線番＋線番(2)行コンポーネント（工事種別：変更・残置 / 工事種別：撤去（使用中））
 ...
 */
export default class LineConnectionSearchForm extends Component {
    ...
    render() {
        ...
        const secondIdfPatchCableSequence = getSelectedEndIdfPatchCables(lineConnections, secondSeriesNo);
        const firstRelPatchCableSequences = getSelectedRelPatchCables(lineConnections, firstSeriesNo);
        const secondRelPatchCableSequences = getSelectedRelPatchCables(lineConnections, secondSeriesNo);
            ...
            <div>
                <InputForm.Row>
                    <InputForm.Col label="線番" columnCount={1} isRequired>
                        ...
                        {firstRelPatchCableSequences && firstRelPatchCableSequences.length > 0 &&                    
                            <RelConnectLabelForm
                                className="mb-1"
                                patchCableSequences={firstRelPatchCableSequences}
                                
                            />
                        }                       
                        <PatchCableLabelForm
                            label="IDF線番"
                            patchCableSequence={firstIdfPatchCableSequence}
                        />
                    </InputForm.Col>
                </InputForm.Row>
                {hasWireType&&
                    <InputForm.Row>
                        <InputForm.Col label="線番(2)" columnCount={1} isRequired>
                            ...
                            />
                            {secondRelPatchCableSequences && secondRelPatchCableSequences.length > 0 &&                    
                                <RelConnectLabelForm
                                    className="mb-1"
                                    patchCableSequences={secondRelPatchCableSequences}
                                    
                                />
                            }
                            <PatchCableLabelForm
                                label="IDF線番"
                                patchCableSequence={secondIdfPatchCableSequence}
                            />
                        </InputForm.Col>
                    </InputForm.Row>
                }
