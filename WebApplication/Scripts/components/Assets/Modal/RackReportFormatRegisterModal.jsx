 ...
 * 
 * ラック帳票フォーマット登録モーダル Reactコンポーネント
 * 
 */
'use strict';
 ...
 */
export default class RackReportFormatRegisterModal extends Component {
     ...
     */
    onSave() {
        const { file, enterprise, allowOverwriting, checkFormat } = this.state;
    ...
    saveFormat(data) {
        this.setState({ isSaving: true }, () => {
            sendData(EnumHttpMethod.post, '/api/embeddedReport/setFormat', data, (result, networkError) => {
    ...
    render() {
        ...
        const { file, enterprise, allowOverwriting, checkFormat, message, isSaving } = this.state;
        ...
        return (
            <Modal show={show} backdrop="static" onHide={() => this.onCancel()}>
                ...
                <Modal.Body>
                    ...
                    <Checkbox
                        checked={checkFormat}
                        onClick={() => this.setState({ checkFormat: !checkFormat })}
                    >
                        フォーマットに無効な詳細項目が存在する場合は登録しない
                    </Checkbox>
                    <MessageModal
                        show={message.show}
                        title={message.title}
...
RackReportFormatRegisterModal.propTypes = {
    show: PropTypes.bool,
